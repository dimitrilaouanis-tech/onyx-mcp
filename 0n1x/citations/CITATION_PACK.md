...
There are three things broken with basically every visibility score on the market — Profound, Bluefish, Peec, Scrunch, Evertune, all of them. Number one: the weights are hidden. They give you a score from zero to a hundred, but they won't tell you the formula. Number two: it's a sample size of one. Answer engines are non-deterministic — even at temperature zero you don't get the same output twice; that's documented in the research, arXiv 2408.04667. And a 2026 study by Sielinski — arXiv 2603.08924 — found that the brands an engine mentions overlap only forty-five to fifty-nine percent from one day to the next. So if a tool runs your query once and hands you a number, that number is noise. Number three: nobody signs it. There's no cryptographic signature, which means anyone could change the number and you'd never know.
...
If you want to actually improve your AI visibility, the research is clear. The GEO paper, Aggarwal et al., arXiv 2311.09735, tested this: adding quotations, statistics, and citing sources were the strongest levers — lifting the position-adjusted visibility metric by roughly forty-three, thirty-four, and twenty-nine percent, up to about forty percent overall. And Google's AI Overviews don't rank blue links — per sworn DOJ testimony they ground through FastSearch and RankEmbed based on entity strength. Not your backlinks. Your entity.
...
▶ STATS & SOURCES
• temperature=0 is not deterministic for LLMs (arXiv:2408.04667)
• brand-mention sets overlap only 45–59% day-to-day (Sielinski, arXiv:2603.08924)
• Top GEO levers ~ +43% quotations / +34% statistics / +29% cite-sources on position-adjusted word count, up to ~40% (GEO paper, arXiv:2311.09735)
...
**What actually moves the needle.** The GEO study (Aggarwal et al., arXiv:2311.09735): the top content levers — quotations (~+43%), statistics (~+34%), cite-sources (~+29%) on the position-adjusted word-count metric, up to ~40% overall. And don't over-index on backlinks — per sworn DOJ testimony, Google AI Overviews ground via FastSearch/RankEmbed entity strength, not blue-link ranking.
...
**The non-determinism problem (the whole ballgame).** Answer engines are not deterministic even at `temperature=0` (arXiv:2408.04667); a 2026 study (Sielinski, arXiv:2603.08924) found **brand-mention sets overlap only 45–59% between consecutive days** (cited-source sets 34–42%). So any score from one query is noise dressed as a metric.
...
**The levers that move it** (GEO paper, Aggarwal et al., arXiv:2311.09735): quotations **~+43%**, statistics **~+34%**, cite-sources **~+29%** on the position-adjusted word-count metric (up to ~40% overall). Keyword stuffing does worse than baseline.
...
**Reply A (which AEO tool should I buy?):** For a CMO dashboard the funded ones are fine, but know what you're buying: a single hidden-weight score with no sample size. Engines are non-deterministic even at temp=0 (arXiv:2408.04667); brand-mention sets overlap only 45–59% day-to-day (Sielinski, arXiv:2603.08924). Demand: runs per prompt, a CI, and the formula. For an auditable sanity-check, `onyx_aeo_score` publishes weights, runs N≥3 with a 95% CI, and signs the output.

**Reply B (why different results every time?):** Because LLM engines are non-deterministic — expected, not a bug. Even temp=0 isn't deterministic (arXiv:2408.04667); brand-mention sets overlap only 45–59% day-to-day (Sielinski, arXiv:2603.08924). Fix is statistical: query each prompt N≥3 (ideally 10+), bootstrap a 95% CI, report a *range*. `onyx_aeo_score` already does N>1 + CI + signature; otherwise it's ~20 lines around your API calls + `numpy.percentile`.
...
*Why it swings:* it's the system (MoE routing, batched non-determinism, retrieval reordering), not your code. Even temp=0 isn't deterministic (arXiv:2408.04667). Sielinski (arXiv:2603.08924): brand-mention sets overlap only 45–59% between consecutive days.
...
`- [onyx_aeo_score (0n1x)]([GitHub repo]) — The auditable AEO score: published formula 100·(0.35·Presence + 0.30·WeightedSoV + 0.25·CitationRate + 0.10·Sentiment), N≥3 runs with a published 95% CI, and an Ed25519 signature on every payload. Built because answer engines are non-deterministic even at temp=0.`
